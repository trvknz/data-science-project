...
// Generate report
let report = `"Cluster", "Phonetic Code", "Complexity", "Avg. Count", "Highest Count", "Lowest Count", "Names"\n`;

clusters.forEach((cluster, idx) => {
    report += `${idx + 1}, ${cluster.confusionScore.toFixed(2)},`;
    report += `"${cluster.phoneticCode}",`;
    report += `${cluster.avgComplexity.toFixed(2)},`;
    report += `${cluster.avgValue.toFixed(0)},`;
    report += `${Math.max(...cluster.entries.map(e => e.value))},`;
    report += `${Math.min(...cluster.entries.map(e => e.value))},`;
    report += `"${cluster.names.join(' | ')}"\n`;
});

// Save results
fs.writeFileSync('cluster_sheet.txt', report);
